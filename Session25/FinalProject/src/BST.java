...
public class BST {
    ...
    private BSTNode deleteRecursive(BSTNode node, String nim) {
        if (node == null) {
            return node;
        }

        int perbandingan = nim.compareTo(node.data.getNim());

        if (perbandingan < 0) {
            node.left = deleteRecursive(node.left, nim);
        }
        else if (perbandingan > 0) {
            node.right = deleteRecursive(node.right, nim);
        }
        else {
            if (node.left == null) {
                return node.right;
            ...
            node.right = deleteRecursive(node.right, node.data.getNim());
        }

        return node;
    }

    // Fungsi pembantu untuk mencari nilai terkecil
    private Mahasiswa minValue(BSTNode node) {
        Mahasiswa minData = node.data;
